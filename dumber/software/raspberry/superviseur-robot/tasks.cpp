...
void Tasks::Init() {
    ...
    if (err = rt_task_create(&th_checkWD, "th_checkWD", 0, PRIORITY_TCHECKWD, 0)) {
        cerr << "Error task create: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }
    ...
    if (err = rt_task_create(&th_img, "th_img", 0, PRIORITY_TIMG, 0)) {
        cerr << "Error task create: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }
    ...
    while (1) {
        ...
        if (!WD) {
            ...
            if (msgSend->GetID() == MESSAGE_ANSWER_ACK) {
                ...
                //rt_mutex_release(&mutex_monitor);
                int err;
                if (err = rt_task_start(&th_checkWD, (void(*)(void*)) & Tasks::CheckRobotWithWD, this)) {
                    cerr << "Error task start: " << strerror(-err) << endl << flush;
                    rt_mutex_acquire(&mutex_monitor, TM_INFINITE);
...
void Tasks::OpenCamera(void *arg) {
    ...
    /**************************************************************************************/
    /* The task openCamera starts here                                                  */
    /**************************************************************************************/
    while (1) {
...
void Tasks::CheckRobotWithWD(void *arg) {
    cout << "Start " << __PRETTY_FUNCTION__ << endl << flush;
    // Synchronization barrier (waiting that all tasks are starting)
    rt_sem_p(&sem_barrier, TM_INFINITE);
    //Message *msgSend;
